...
function buildHTML(stats: ShareStats): string {
  ...
  const busiest = stats.busiestYear > 0 ? String(stats.busiestYear) : '\u2014';
...
function draw(){
  ...
  var glow=ctx.createRadialGradient(W/2,300,0,W/2,300,820);
  ...
  ctx.font='800 260px -apple-system,system-ui,sans-serif';
  ctx.textAlign='center';
  ctx.fillText(String(D.n),W/2,586);

  // "countries explored"
  ctx.fillStyle='rgba(148,163,184,0.68)';
  ctx.font='500 66px -apple-system,system-ui,sans-serif';
  ctx.fillText('countries explored',W/2,668);

  // "so far" + amber accent period
  ctx.font='500 66px -apple-system,system-ui,sans-serif';
  ctx.textAlign='left';
  var sf='so far';
  ...
  ctx.fillStyle='rgba(148,163,184,0.68)';
  ctx.fillText(sf,sfX,744);
  ctx.fillStyle='#F59E0B';
  ctx.fillText('.',sfX+sfW,744);
  ...
  function statRow(y,val,label){
    var mid=y+rH/2;
    // Large value — left aligned
    ctx.fillStyle='#F8FAFC';
    ctx.font='bold 108px -apple-system,system-ui,sans-serif';
    ctx.textAlign='left';ctx.textBaseline='middle';
    ctx.fillText(val,P,mid);
    // Label — right aligned, muted
    ctx.fillStyle='rgba(100,116,139,0.88)';
    ctx.font='400 50px -apple-system,system-ui,sans-serif';
    ctx.textAlign='right';
    ctx.fillText(label,W-P,mid);
  }

  divider(sy);
  statRow(sy, String(D.co)+'/7', 'continents');
  divider(sy+rH);
  statRow(sy+rH, String(D.cy), 'countries in '+String(D.yr));
  divider(sy+rH*2);
  statRow(sy+rH*2, String(D.pct)+'%', 'of the world');
  divider(sy+rH*3);
  statRow(sy+rH*3, String(D.by), 'busiest year');
  divider(sy+rH*4);
