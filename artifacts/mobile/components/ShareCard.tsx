...
function bigPlane(x,y,rot,sz){
  ...
  // fL = half-fuselage length; shorter than sz gives realistic body/wingspan ratio
  var fL=sz*0.52;
  // Fuselage — nose at -fL, tail at +fL
  ctx.beginPath();
  ctx.moveTo(0,-fL);
  ctx.bezierCurveTo(sz*0.10,-fL*0.88,sz*0.12,-fL*0.40,sz*0.12,fL*0.30);
  ctx.bezierCurveTo(sz*0.10,fL*0.78,sz*0.05,fL*0.96,0,fL);
  ctx.bezierCurveTo(-sz*0.05,fL*0.96,-sz*0.10,fL*0.78,-sz*0.12,fL*0.30);
  ctx.bezierCurveTo(-sz*0.12,-fL*0.40,-sz*0.10,-fL*0.88,0,-fL);
  ctx.closePath();ctx.fill();
  // Left main wing (far side — larger)
  ctx.beginPath();
  ctx.moveTo(-sz*0.12,-fL*0.15);ctx.lineTo(-sz*0.96,fL*0.52);
  ctx.lineTo(-sz*0.88,fL*0.84);ctx.lineTo(-sz*0.12,fL*0.56);
  ctx.closePath();ctx.fill();
  // Right main wing (near side — slightly smaller)
  ctx.beginPath();
  ctx.moveTo(sz*0.12,-fL*0.15);ctx.lineTo(sz*0.82,fL*0.46);
  ctx.lineTo(sz*0.74,fL*0.76);ctx.lineTo(sz*0.12,fL*0.50);
  ctx.closePath();ctx.fill();
  // Left engine pod
  ctx.save();ctx.translate(-sz*0.58,fL*0.40);ctx.rotate(0.26);
  ctx.beginPath();ctx.ellipse(0,0,sz*0.068,sz*0.20,0,0,Math.PI*2);ctx.fill();
  ctx.restore();
  // Right engine pod
  ctx.save();ctx.translate(sz*0.49,fL*0.36);ctx.rotate(0.26);
  ctx.beginPath();ctx.ellipse(0,0,sz*0.058,sz*0.17,0,0,Math.PI*2);ctx.fill();
  ctx.restore();
  // Vertical tail fin
  ctx.beginPath();
  ctx.moveTo(-sz*0.03,fL*0.44);ctx.lineTo(-sz*0.08,fL*0.06);
  ctx.lineTo(sz*0.06,fL*0.16);ctx.lineTo(sz*0.05,fL*0.56);
  ctx.closePath();ctx.fill();
  // Left horizontal stabilizer
  ctx.beginPath();
  ctx.moveTo(-sz*0.12,fL*0.76);ctx.lineTo(-sz*0.44,fL*0.90);
  ctx.lineTo(-sz*0.42,fL*0.98);ctx.lineTo(-sz*0.12,fL*0.88);
  ctx.closePath();ctx.fill();
  // Right horizontal stabilizer
  ctx.beginPath();
  ctx.moveTo(sz*0.12,fL*0.74);ctx.lineTo(sz*0.38,fL*0.86);
  ctx.lineTo(sz*0.36,fL*0.94);ctx.lineTo(sz*0.12,fL*0.84);
  ctx.closePath();ctx.fill();
  ctx.restore();
}
...
function drawCard(land){
  ...
  // ── Top bar ──────────────────────────────────────────────────────────────────
  // App logo in a circle
  var logoEl=document.getElementById('applogo');
  if(logoEl && logoEl.complete && logoEl.naturalWidth>0){
    ctx.save();
    ctx.beginPath();ctx.arc(72,62,40,0,Math.PI*2);ctx.clip();
    ctx.drawImage(logoEl,32,22,80,80);
    ctx.restore();
  } else {
    // Fallback: navy circle with amber plane icon
