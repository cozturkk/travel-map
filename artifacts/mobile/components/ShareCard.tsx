...
// Smooth organic blob (bezier-approximated ellipse)
...
// Large commercial airplane silhouette (top-down view)
function bigPlane(x,y,rot,sz){
  ctx.save();ctx.translate(x,y);ctx.rotate(rot);
  ctx.fillStyle='rgba(228,175,52,0.97)';
  // Fuselage (thin ellipse, nose=top)
  ctx.beginPath();ctx.ellipse(0,0,sz*0.115,sz,0,0,Math.PI*2);ctx.fill();
  // Main wings (swept back)
  ctx.beginPath();
  ctx.moveTo(-sz*0.06, sz*0.08);
  ctx.lineTo(-sz*0.82, sz*0.28);
  ctx.lineTo(-sz*0.78, sz*0.42);
  ctx.lineTo(-sz*0.06, sz*0.36);
  ctx.lineTo( sz*0.06, sz*0.36);
  ctx.lineTo( sz*0.78, sz*0.42);
  ctx.lineTo( sz*0.82, sz*0.28);
  ctx.lineTo( sz*0.06, sz*0.08);
  ctx.closePath();ctx.fill();
  // Horizontal stabiliser (tail)
  ctx.beginPath();
  ctx.moveTo(-sz*0.06, sz*0.72);
  ctx.lineTo(-sz*0.38, sz*0.82);
  ctx.lineTo(-sz*0.36, sz*0.94);
  ctx.lineTo(-sz*0.06, sz*0.90);
  ctx.lineTo( sz*0.06, sz*0.90);
  ctx.lineTo( sz*0.36, sz*0.94);
  ctx.lineTo( sz*0.38, sz*0.82);
  ctx.lineTo( sz*0.06, sz*0.72);
  ctx.closePath();ctx.fill();
  // Engine pods
...
bg.addColorStop(0,'#0c1b2e');bg.addColorStop(0.42,'#0a1726');bg.addColorStop(1,'#07101d');
ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);

// Soft radial atmosphere glow behind globe
...
// ── Globe — everything inside clipped ─────────────────────────────────────────
ctx.save();
...
// ── Continent blobs (amber) ───────────────────────────────────────────────────
ctx.fillStyle='#c96318';
// Africa — large central
...
// North America — left edge (partially visible)
...
// ── Clouds (soft light-blue at globe edges) ───────────────────────────────────
ctx.fillStyle='rgba(192,214,255,0.80)';
cloud(GX-GR*0.58, GY-GR*0.54, 38);  // upper-left
cloud(GX-GR*0.70, GY+GR*0.38, 32);  // lower-left
cloud(GX+GR*0.54, GY-GR*0.52, 32);  // upper-right
cloud(GX+GR*0.60, GY+GR*0.36, 28);  // lower-right

// ── Dashed flight paths ───────────────────────────────────────────────────────
ctx.save();
ctx.setLineDash([13,17]);ctx.lineWidth=4.2;ctx.strokeStyle='rgba(218,108,18,0.82)';ctx.lineCap='round';
// Path 1: lower-left → arcs up through globe center → right side
ctx.beginPath();
ctx.moveTo(GX-GR*0.80, GY+GR*0.56);
ctx.quadraticCurveTo(GX-GR*0.08, GY-GR*0.56, GX+GR*0.88, GY+GR*0.08);
ctx.stroke();
// Path 2: upper area → curves right and down
ctx.beginPath();
...
// ── Large airplane (on path 1, upper quadrant) ────────────────────────────────
// Positioned at the apex of path 1, nose pointing upper-right
...
// Small plane icon inside the badge
...
ctx.restore();
ctx.fillStyle='rgba(228,242,255,0.94)';ctx.font='bold 42px -apple-system,system-ui,Arial,sans-serif';ctx.textAlign='left';
...
// ── Stat boxes ────────────────────────────────────────────────────────────────
var bY=hY+62, bH=272, bGap=22, bW=Math.floor((W-120-bGap)/2);

// Left — countries visited
...
ctx.fillText(cStr,88,bY+22+Math.round(cFs*0.74));
ctx.fillStyle='rgba(150,202,255,0.72)';ctx.font='400 43px -apple-system,system-ui,Arial,sans-serif';
...
ctx.fillText(pStr,rBx+28,bY+22+Math.round(pFs*0.74));
...
ctx.fillStyle='rgba(255,192,110,0.72)';ctx.font='400 43px -apple-system,system-ui,Arial,sans-serif';
ctx.fillText('of the',rBx+28,bY+bH-56);
ctx.fillText('world',rBx+28,bY+bH-10);

// ── FLAGS COLLECTED ───────────────────────────────────────────────────────────
var flY=bY+bH+82;
ctx.textAlign='left';ctx.fillStyle='rgba(130,172,224,0.52)';
...
if(flags.length>maxF){
  ctx.fillStyle='rgba(130,172,224,0.52)';ctx.font='400 38px -apple-system,system-ui,Arial,sans-serif';
  ctx.fillText('+'+(flags.length-maxF)+' more',60,flY+56+2*96+fSz+10);
}

// ── Footer ────────────────────────────────────────────────────────────────────
ctx.fillStyle='rgba(255,255,255,0.07)';ctx.fillRect(0,H-84,W,84);
ctx.fillStyle='rgba(170,206,255,0.36)';ctx.font='400 34px -apple-system,system-ui,Arial,sans-serif';ctx.textAlign='center';
